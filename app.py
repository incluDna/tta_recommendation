...
THEME_DESC = {
    "Defensive Driving": (
        "อุบัติเหตุที่เกิดจากพฤติกรรมการขับขี่ไม่ระมัดระวัง เช่น ตัดหน้า"
        " ซ้อนท้าย ฝ่าไฟแดง หรือเปลี่ยนช่องทางกะทันหัน"
    ),
    "Focus & Attention": (
    ...
    "Road & Vehicle Safety": (
        "อุบัติเหตุที่เกิดจากสภาพถนนและยานพาหนะ เช่น ถนนลื่น หลุมบ่อ ยางแตก"
        " หรืออุปกรณ์ชำรุด"
    ),
    "Speed Awareness": (
...
with st.sidebar:
    ...
    if use_pii:
        ...
        if bank_files:
            ...
            run_joined_btn = st.button(
                "▶️ รัน merged excel", use_container_width=True, type="primary"
            )
            ...
            if run_joined_btn:
                if "result" not in st.session_state:
                    ...
                    st.error(
                        "⚠️ กรุณากด '▶️ รัน Analysis' ใหม่อีกครั้ง"
                        " (analysis.py อัปเดตแล้ว)"
                    )
                else:
                    res = st.session_state["result"]
                    personalized = build_personalized_output(
                        res["raw_with_id"], res["df"]
                    )
...
st.caption(f"ไฟล์: **{sel_file}** | Sheet: **{sel_sheet}**")

# ─────────────────────────────────────────────────────────────────
# FILTER: เลือกเดือน / พื้นที่
# ─────────────────────────────────────────────────────────────────
f1, f2 = st.columns(2)
...
with f2:
    if COL_AREA in df_full.columns:
        area_opts = sorted(
            df_full[COL_AREA].dropna().unique().tolist(), key=natural_sort_key
        )
...
st.caption(
    f"Data: **{len(df):,}** / {len(df_full):,} | พื้นที่: **{len(area)}** แห่ง"
)
...
with tab_dash:
    ...
    top_theme = (
        df["campaign_theme"].value_counts().idxmax()
        if df["campaign_theme"].notna().any()
        else "-"
    )
...
with tab_insight:
    ...
    if "rider_exp_group" in df.columns and COL_SEVERITY in df.columns:
        ...
        highest_group = (
            exp_pct["หยุดงานเกิน3วัน"].idxmax()
            if "หยุดงานเกิน3วัน" in exp_pct.columns
            else "-"
        )
        highest_value = (
            exp_pct["หยุดงานเกิน3วัน"].max()
            if "หยุดงานเกิน3วัน" in exp_pct.columns
            else 0.0
        )
        ...
        sev_order = [c for c in SEV_ORDER if c in exp_pct.columns]
        ct = exp_pct[sev_order]

        styled = (
            ct.style.background_gradient(
                cmap="Reds", subset=["หยุดงานเกิน3วัน"]
            )
    ...
    if COL_SEVERITY in df.columns and "campaign_theme" in df.columns:
        ct2 = (
            pd.crosstab(
                df["campaign_theme"], df[COL_SEVERITY], normalize="index"
            )
            * 100
        ).round(1)
        top_theme = (
            ct2["หยุดงานเกิน3วัน"].idxmax()
            if "หยุดงานเกิน3วัน" in ct2.columns
            else "-"
        )
        top_value = (
            ct2["หยุดงานเกิน3วัน"].max()
            if "หยุดงานเกิน3วัน" in ct2.columns
            else 0.0
        )
        st.info(
...
with tab_campaign:
    if area.empty:
        ...
        for area_name, row in area.iterrows():
            ...
            c2.metric(
                "Confidence", f"{conf:.1f}%" if not np.isnan(conf) else "-"
            )
            c3.metric(
                "Recommended (อิงจากความถี่และความรุนแรง)",
                f"{THEME_ICON.get(rec,'')} {rec}",
            )
...
""",
        ...
        tbl = tbl.sort_values(
            COL_AREA, key=lambda s: s.map(natural_sort_key)
        )
        st.caption(
            "💡 คลิกหัวตาราง (เช่น 'พื้นที่' หรือ 'risk_score') เพื่อเรียงลำดับได้"
        )
        st.dataframe(
        ...
        with st.expander("📖 ดูรายละเอียด: สาเหตุในแต่ละ theme"):
            st.caption(
                "ใช้ตอนคิด message campaign —"
                " เห็นคำจริงที่ระบบจัดกลุ่มไว้ในแต่ละ theme"
            )
            ...
            mapping_df = (
                pd.DataFrame([
                    {"สาเหตุ (cause)": cause, "Theme": theme}
                    for cause, theme in CAUSE_THEME.items()
                ])
            ...
            selected_theme = st.selectbox(
                "เลือก Theme ที่จะดู",
                ["ทั้งหมด"] + sorted(mapping_df["Theme"].unique().tolist()),
            )
...
with tab_pred:
    ...
    if not pred_file.exists():
        ...
        with pf1:
            sel_pred_q = st.selectbox(
                "📅 เลือกไตรมาสพยากรณ์", options=["Q3", "Q4"], index=0
            )
        with pf2:
            pred_area_opts = sorted(
                df_pred["พื้นที่"].dropna().unique().tolist(),
                key=natural_sort_key,
            )
            sel_pred_area = st.selectbox(
                "📍 เลือกพื้นที่", options=["ทั้งหมด"] + pred_area_opts, index=0
            )

        df_p_filtered = df_pred[df_pred["Quarter"] == sel_pred_q]
        if sel_pred_area != "ทั้งหมด":
            df_p_filtered = df_p_filtered[
                df_p_filtered["พื้นที่"] == sel_pred_area
            ]
        ...
        top_predicted_theme = (
            df_p_filtered.groupby("Predicted_Theme")[
        ...
        high_trend_count = (
            len(
                df_p_filtered[df_p_filtered["Q1-Q2_Trend_Multiplier"] > 1.1][
                    "พื้นที่"
                ].unique()
            )
            if "Q1-Q2_Trend_Multiplier" in df_p_filtered.columns
            else 0
        )
        ...
        pk3.metric(
            "จำนวนพื้นที่ที่มี Trend เสี่ยงพุ่งสูง (YoY > 1.1x)",
            f"{high_trend_count} พื้นที่",
        )
        ...
        with c_pred2:
            st.markdown(
                f"#### 🔝 Top 5 สาเหตุย่อยที่มีแนวโน้มเกิดสูงสุด ({sel_pred_q})"
            )
            top_causes_pred = (
                df_p_filtered.groupby(
                    ["Sub_Cause (สาเหตุย่อย)", "Predicted_Theme"],
                    as_index=False,
                )["Estimated_Cause_Probability (%)"]
                .mean()
                .sort_values(
                    by="Estimated_Cause_Probability (%)", ascending=False
                )
                .head(5)
            )
        ...
        st.dataframe(
            df_p_filtered.style.background_gradient(
                subset=["Estimated_Cause_Probability (%)"], cmap="YlOrRd"
            ),
            use_container_width=True,
            hide_index=True,
        )
...
with tab_insurance:
    ...
    if not area.empty:
        ...
        st.dataframe(
            ins_tbl.style.map(_color_pri, subset=["priority_level"])
